...
test('team invitations can be accepted', function () {
    ...
    $response = $this
        ...
        ->get(route('team_invitations.accept', $invitation));
...
test('team invitations can be declined by the invited user', function () {
    ...
    $response = $this
        ->actingAs($invitedUser)
        ->delete(route('team_invitations.decline', $invitation));

    $response->assertRedirect(route('invitations.index', $invitedUser->currentTeam));
